...
export async function GET(request: NextRequest) {
  return withAuth(request, async (authUser) => {
    try {
      ...
      const exportType = searchParams.get('format') || 'json'; // json, csv
      ...
      if (exportType === 'csv') {
        // For CSV, we'd typically convert to a CSV string
        // but for this API endpoint we'll return the JSON data
        // The actual CSV conversion happens on the client side
      ...
      return NextResponse.json(
        { success: false, message: 'Failed to generate report for export' },
        { status: 500 }
      );
    }
  });
