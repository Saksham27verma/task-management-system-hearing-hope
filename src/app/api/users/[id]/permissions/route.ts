...
export async function GET(
  ...
  return withPermission(request, async (user) => {
    try {
      ...
      console.log(`Retrieved ${permissions.length} permissions for user ${params.id}`);
      ...
      console.error(`Error fetching permissions for user ${params.id}:`, error);
      return NextResponse.json(
...
export async function PUT(
  ...
  return withPermission(request, async (user) => {
    try {
      ...
      console.log(`Updating permissions for user ${params.id}`, { customPermissions, permissionGroups });
      ...
      if (Array.isArray(customPermissions)) {
        console.log(`Setting ${customPermissions.length} custom permissions for user ${params.id}`);
        userToUpdate.customPermissions = customPermissions;
      }
      
      // Update permission groups if provided
      if (Array.isArray(permissionGroups)) {
        console.log(`Setting ${permissionGroups.length} permission groups for user ${params.id}`);
        userToUpdate.permissionGroups = permissionGroups;
      }
      
      // Save updated user
      await userToUpdate.save();
      console.log(`Successfully updated permissions for user ${params.id}`);
      ...
      console.error(`Error updating permissions for user ${params.id}:`, error);
      return NextResponse.json(
